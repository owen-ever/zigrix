...
현재 Zigrix 실체는 `<orchestration-root>/` 아래에 있다.
...
- `<openclaw-workspace-root>-pro-zig/projects`
- `<openclaw-agents-root>`
...
- `ROOT = Path('<openclaw-workspace-root>')`
- `PROJECTS_ROOT = '<openclaw-workspace-root>-pro-zig/projects'`
- `AGENTS_STATE_DIR = Path('<openclaw-agents-root>')`
