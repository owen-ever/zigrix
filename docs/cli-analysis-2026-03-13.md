...
현재 Zigrix 실체는 `<OPENCLAW_HOME>/workspace/orchestration/` 아래에 있다.
...
- `<OPENCLAW_HOME>/workspace/projects`
...
- 특정 agentId 체계(`orchestrator-agent`, `frontend-agent`, `qa-agent` 등)가 존재함
- 특정 workspace 레이아웃이 있음
- 메인/서브에이전트 역할 분담이 이미 정해져 있음

즉, 지금은 “범용 CLI”가 아니라 **owner 환경에 맞춘 orchestration runtime**에 가깝다.
...
- `ROOT = Path('<OPENCLAW_HOME>/workspace')`
- `PROJECTS_ROOT = '<OPENCLAW_HOME>/workspace/projects'`
- `AGENTS_STATE_DIR = Path('<OPENCLAW_HOME>/agents')`
...
현재는 `orchestrator-agent`, `qa-agent`, `frontend-agent`, `backend-agent`, `system-agent`, `security-agent` 중심이다.
