...
describe('dashboard completed-task behavior regressions', () => {
  ...
  it('keeps conversation available when gateway fails but at least one local fallback succeeds', async () => {
    ...
    const missingSessionKey = `agent:qa-a:subagent:${missingSessionId}`;
    ...
    const qaSessionsDir = path.join(home.agentsDir, 'qa-a', 'sessions');
    ...
    writeTaskMeta(home.tasksDir, taskId, {
      ...
      workerSessions: {
        ...
        'qa-a': { sessionKey: missingSessionKey, sessionId: null, status: 'done' },
      },
    });
